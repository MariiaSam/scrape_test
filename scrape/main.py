...
def parse_data():
    ...
    if html_doc.status_code == 200:
        soup = BeautifulSoup(html_doc.content, 'html.parser') # парсинг даних з HTML коду
        
        books = soup.select('section')[0].find_all('article', attrs={'class': 'product_pod'}) # Знаходимо всі книги
        for book in books: # картки кожної книги починаємо діставати інформацію.Обкладинку книги дістаємо з атрибуту src тегу img
            
            img_url = f"{url}{book.find('img')['src']}"
            rating = rate_to_number.get(book.find('p', attrs={'class': 'star-rating'})['class'][1])
            
            # назву книги ми отримуємо з атрибуту title тегу a.
            title = book.find('h3').find('a')['title']
            
            # Оскільки всередині картки книги два теги a, ми конкретизуємо пошук. І останнім ми отримуємо ціну книги
